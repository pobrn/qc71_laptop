...
static struct platform_device *qc71_platform_dev = NULL;
static struct device *qc71_hwmon_dev = NULL;
...
static bool battery_hook_registered = false,
	    lightbar_led_registered = false;
static int  wmi_handlers_installed = 0;
...
static int
qc71_lightbar_get_status(void)
{
	return ec_read_byte(LIGHTBAR_CONTROL_ADDR);
}

static int
qc71_lightbar_write_ctrl(u8 ctrl)
{
	return ec_write_byte(LIGHTBAR_CONTROL_ADDR, ctrl);
}

static int
qc71_lightbar_switch(u8 mask, bool on)
{
	int status;
...
static int
qc71_lightbar_set_color_level(u8 color, u8 level)
{
...
static int
qc71_lightbar_set_rainbow_mode(bool on)
{
...
#define DEFINE_FUNC_show(name, addr) \
...
#define DEFINE_FUNC_store(name, addr) \
...
#define DEFINE_BOTH(name, addr) \
DEFINE_FUNC_show(name, addr) \
DEFINE_FUNC_store(name, addr)

DEFINE_BOTH(ctrl_1, CTRL_1_ADDR)
DEFINE_BOTH(ctrl_2, CTRL_2_ADDR)
DEFINE_BOTH(ctrl_3, CTRL_3_ADDR)
DEFINE_BOTH(ctrl_4, CTRL_4_ADDR)
DEFINE_BOTH(ap_bios_byte, AP_BIOS_BYTE_ADDR)
DEFINE_BOTH(bios_ctrl_3, BIOS_CTRL_3_ADDR)
DEFINE_BOTH(fan_ctrl, FAN_CTRL_ADDR)

/*
DEFINE_BOTH(fan_minspeed, FAN_MINSPEED_ADDR)
DEFINE_BOTH(fan_mintemp, FAN_MINTEMP_ADDR)
DEFINE_BOTH(fan_extraspeed, FAN_EXTRASPEED_ADDR)
*/
...
static DEVICE_ATTR_RW(ctrl_1);
static DEVICE_ATTR_RW(ctrl_2);
static DEVICE_ATTR_RW(ctrl_3);
static DEVICE_ATTR_RW(ctrl_4);
static DEVICE_ATTR_RW(ap_bios_byte);
static DEVICE_ATTR_RW(bios_ctrl_3);
static DEVICE_ATTR_RW(fan_ctrl);

/*
static DEVICE_ATTR_RW(fan_minspeed);
static DEVICE_ATTR_RW(fan_mintemp);
static DEVICE_ATTR_RW(fan_extraspeed);
*/
...
static umode_t
qc71_hwmon_is_visible(const void *data, enum hwmon_sensor_types type,
...
static int
qc71_hwmon_read(struct device *device, enum hwmon_sensor_types type,
...
static struct hwmon_ops
qc71_hwmon_ops = {
	.is_visible  = qc71_hwmon_is_visible,
	.read        = qc71_hwmon_read,
};

static struct hwmon_chip_info
qc71_hwmon_chip_info = {
	.ops  = &qc71_hwmon_ops,
...
qc71_lightbar_led_get_brightness(struct led_classdev *led_cdev)
{
...
static int
qc71_lightbar_led_set_brightness(struct led_classdev *led_cdev, enum led_brightness value)
{
...
static void
qc71_wmi_event_d2_handler(union acpi_object *obj)
{
...
static void
qc71_wmi_event_handler(u32 value, void *context)
{
...
#ifdef CONFIG_DMI
static int __init
qc71_dmi_callback(const struct dmi_system_id *id)
{
	pr_info("'%s' found\n", id->ident);
	return 1;
}
...
static void __init
oem_string_walker(const struct dmi_header *dm, void *ptr)
{
	int i, count;
	const u8 *s;
...
parse_bios_version(const char *str)
{
	int bios_version;
...
do_cleanup(void)
{
	...
	case 3: wmi_remove_notify_handler(QC71_WMI_EVENT2_GUID); /* fallthrough */
	case 2: wmi_remove_notify_handler(QC71_WMI_EVENT1_GUID); /* fallthrough */
	case 1: wmi_remove_notify_handler(QC71_WMI_EVENT0_GUID); /* fallthrough */
	default:
		break;
	}
...
static void __exit
qc71_laptop_module_cleanup(void)
{
	do_cleanup();
